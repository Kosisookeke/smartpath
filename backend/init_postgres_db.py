"""
PostgreSQL database initialization script for SmartPath
This script creates all necessary tables and seeds initial data for PostgreSQL
"""
import os
import psycopg2
...
def init_database():
    ...
    if user_count == 0:
        ...
        cursor.execute('''
            ...
            VALUES (%s, %s, %s, %s, %s)
            RETURNING id
        ''', ('admin@smartpath.com', admin_hash, 'Admin User', 'admin', datetime.utcnow()))
        ...
        cursor.execute('''
            INSERT INTO users (email, password_hash, name, role, created_at)
            VALUES (%s, %s, %s, %s, %s)
        ''', ('student@smartpath.com', student_hash, 'Student User', 'student', datetime.utcnow()))
...
Learn to analyze time and space complexity of algorithms.
        ...
        for course in courses_data:
            ...
            ''', (course['title'], course['description'], course['category'], 
                  course['content'], admin_id, datetime.utcnow()))
...
if __name__ == '__main__':
    print("=" * 60)
    print("SmartPath PostgreSQL Database Initialization")
    print("=" * 60)
    try:
        init_database()
    except Exception as e:
